...
public:
    ...
    Whale update(const Whale &best,  double a, double p) const {
        ...
        if (std::uniform_real_distribution<double>(0,1)(rng) < 0.1) {
            Tabu_Search( *cfg_ ,  &offspring , 30 , 10 , 30);
        }
...
Solution Whale_Optimize(const Config& cfg , int num_whales = 20 , int max_iter   = 200 , // 傳入 配置檔案參數 實例 ，群體參數 20 隻預設 ， 200 迭代數
                    vector<double>* GB_Recorder = nullptr , vector<double>* PB_Recorder = nullptr )  // 最佳解cost紀錄、群體平均紀錄(用於觀察群體收斂)
{
    // 初始化種群(隨機生成)
    ...
    for (int iter = 1; iter <= max_iter; ++iter) {
        ...
        for (int i = 0; i < num_whales; ++i) {
            ...
            // 更新
            Whale offspring = cur.update(best, a, p);
            // 若後代更優，替換當前
